...
* NO DIAGRAM FALLBACK: For any question where the corresponding REFERENCE PYQ does NOT contain an image/diagram, keep it entirely text-based by default.
  - EXCEPTION (AUTONOMOUS SVG GENERATION - REQUIRED FOR ORGANIC CHEMISTRY):
    If the topic is inherently visual and a diagram would significantly improve educational clarity (e.g., Physics electric circuits, mechanics force/mass setups, ray optics; Math coordinate graphs and plotting), you are STRICKLY ENCOURAGED to generate a custom-built, highly polished inline SVG diagram (using Option B) representing that system.
